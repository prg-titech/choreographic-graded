...
cabal run enclave-demo             # Enclave isolation demo
...
- **Operation.hs** - Communication primitives (`comm`, `enclave`) for type-safe message passing
...
### 3. Faced Values System (`src/Choreographic/Graded/Faced.hs`)
- **Faced Values** - Location-specific values that can differ between locations (unlike Located values which are uniform)
...
- **Faced Values**: `Faced l a` represents location-specific values that can differ at each location in `l`
- **Communication**: `comm @location` for type-safe message passing between specific locations
- **Enclaves**: Restricted execution contexts for security/isolation
- **IO Operations**: `foreach` allows location-specific IO operations on Faced values
